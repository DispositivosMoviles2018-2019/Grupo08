...
public class ReservaImpl  implements Crud{
    ...
    public Reserva getReserva(String id) {
        Cursor cursor = db.query(ConexionSqliteHelper.TABLA_RESERVA,columnas, ConexionSqliteHelper.CAMPO_PLACA + "=?",new String[] {String.valueOf(id)},null,null,null,null);
        if(cursor != null)
            cursor.moveToFirst();
        Reserva reserva = new Reserva(cursor.getString(0),
                cursor.getString(1),
                cursor.getString(2) ,
                cursor.getString(3),
                cursor.getString( 4),
